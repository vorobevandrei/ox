...
# Global constant for maximum allowed output length.
MAX_OUT_LENGTH = 2048
...
def grep(pattern: str, paths: list[str], tool_context: ToolContext) -> str:
    ...
    """
    # Use the global MAX_OUT_LENGTH as the maximum number of characters in the final grep output.
    result_lines = []
    ...
    combined_output = "\n".join(result_lines)
    if len(combined_output) > MAX_OUT_LENGTH:
        suffix = "\n[Output truncated]"
        combined_output = combined_output[:MAX_OUT_LENGTH - len(suffix)] + suffix
    if combined_output:
...
def tree(path: str, tool_context: ToolContext, max_depth: int = 1, include_files: bool = False,
         max_output: int = MAX_OUT_LENGTH) -> str:
    """
